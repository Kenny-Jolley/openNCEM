...
# from ...ncempy.algo.distortion import filter
...
class MaskDialog(QtWidgets.QDialog):
    ...
    def __init__(self, parent):
        ...
        self.roi = pg.PolyLineROI([[int(0.25 * parent.data.shape[2]), int(0.25 * parent.data.shape[1])],
                                   [int(0.75 * parent.data.shape[2]), int(0.25 * parent.data.shape[1])],
                                   [int(0.75 * parent.data.shape[2]), int(0.75 * parent.data.shape[1])],
                                   [int(0.25 * parent.data.shape[2]), int(0.75 * parent.data.shape[1])]], closed=True)
        ...
        self.setToolTip(
            'This dialog allows to create a mask.\nUse the ROI to select the area, which should be removed from affecting the radial profile.')
        self.show()

    def getResult(self):
        im = QtGui.QImage(self.parent.data[self.parent.idx, :, :].shape[0],
                          self.parent.data[self.parent.idx, :, :, ].shape[1], QtGui.QImage.Format_ARGB32)
        im.fill(1)
...
class Main(QtWidgets.QMainWindow):
    ...
    def initUI(self):
        ...
        self.gui_polar['cenopt_btn'].setToolTip(
            'Optimize the current center to minimize the deviation from the mean radial distance of the found positions on one ring.')
        self.gui_polar['dist_txt'].setToolTip(
            'Distortion orders to correct for.\nShould be a comma separated list of integers.')
        self.gui_polar['dists_btn'].setToolTip(
            'Fit the distortions and plot the results to the polar plot.\nThe goal is to minimize the radial deviations.')

        self.gui_radprof['rad_txt'].setToolTip(
            'Parameters for extracting the radial profile:\nr_max is the maximum radial distance considered\ndr is the stepsize on the radial axis\nsigma the width of the Gaussian kernel density estimatior applied\nShould be a comma separated list of three values. Can be left blank to use default values.')
        self.gui_radprof['crct_check'].setToolTip(
            'Toogles whether the distortions fitted above are corrected during extraction of radial profile or not.')
        self.gui_radprof['mask_btn'].setToolTip(
            'Launches a dialog to mask the input image. This allows to select areas, not to be used during extraction of the radial profile.')
        self.gui_radprof['ext_btn'].setToolTip('Extract the radial profile and plot it in the radial profile tab.')
        self.gui_radprof['fitxs_txt'].setToolTip(
            'r values of supporting points in the radial profile, which will be used to fit a power law background.\nShould be a comma separated list of values.')
        self.gui_radprof['fitxs_btn'].setToolTip(
            'Conveniently select the r values for supporting points in the radial profile plot.\nMove the green crosshair and left-click to select a point.\nPress the select button again to stop the selection process.')
        self.gui_radprof['fitxsw_txt'].setToolTip(
            'Range in which points in the radial profile around the supporting points are considered for fitting the background.')
        self.gui_radprof['back_init_txt'].setToolTip(
            'Initial parameters for fitting the power law background (offset + amplitude*r^exponent).\nShould be a list of three comma separated values.\nGood values usually are 1, 1000, -1.')
        self.gui_radprof['fitback_btn'].setToolTip(
            'Fit power law background to the supporting points selected on the radial profile.')
        self.gui_radprof['back_check'].setToolTip(
            'Check to subtract the fitted background.\nMake sure first, that the fitted background fits nicely to the radial profile.\nThis can be judged from the plot, when subtract background is not checked.')
        self.gui_radprof['fit_tbl'].setToolTip(
            'List of peak functions fitted to the radial profile.\nFirst column is the name of the fit function, second column contains comma separated lists of values used as initial guess for the parameters.\nUse the option to plot the initial guess to get the initial parameters right.\nFor now just use \'voigt\' functions, which require lists of four comma separated values as initial parameters.')
    ...
    def on_open_infile(self):
        ...
        '''
        fname = QtWidgets.QFileDialog.getOpenFileName(self, 'Open EMD file',
                                                      filter='EMD files (*.emd);;All files (*.*)')
        try:
            self.femd_in = ncempy.io.emd.fileEMD(fname[0], readonly=True)

            self.gui_file['in_txt'].setText(fname[0])

            self.log('Opening file "{}" ..'.format(fname[0]))
            ...
            self.log('Failed to open EMD file "{}".'.format(fname[0]))
            raise TypeError()

    def on_reopen(self):

        fname = QtWidgets.QFileDialog.getOpenFileName(self, 'Open EMD evaluation file',
                                                      filter='EMD files (*.emd);;All files (*.*)')
        ...
        try:
            ...
            self.log('.. loading data from "{}"-"{}".'.format(self.femd_in.file_hdl.filename,
                                                              self.femd_in.list_emds[0].name))
            ...
            self.gui_localmax['txt_lmax_cinit'].setText(
                '{:g}, {:g}'.format(self.settings['lmax_cinit'][1], self.settings['lmax_cinit'][0]))
            self.gui_localmax['txt_lmax_range'].setText(
                '{:g}, {:g}'.format(self.settings['lmax_range'][0], self.settings['lmax_range'][1]))

            self.gui_polar['dist_txt'].setText(', '.join(map(str, self.settings['ns'])))

            self.gui_radprof['rad_txt'].setText(
                '{:g}, {:g}, {:g}'.format(self.settings['rad_rmax'], self.settings['rad_dr'],
                                          self.settings['rad_sigma']))
            ...
            for i in range(len(self.settings['fit_funcs'])):
                ...
                new_init = QtWidgets.QTableWidgetItem(', '.join(map('{:g}'.format, self.settings['fit_init'][n:n +
                                                                                                               ncempy.algo.math.lkp_funcs[
                                                                                                                   self.settings[
                                                                                                                       'fit_funcs'][
                                                                                                                       i]][
                                                                                                                   1]])))
    ...
    def on_saveEMDFile(self):
        ...
        '''
        fname = QtWidgets.QFileDialog.getSaveFileName(self, 'Save to EMD file',
                                                      filter='EMD files (*.emd);;All files (*.*)')
        ...
        try:
            ...
            hdl = ncempy.eval.ring_diff.put_sglgroup(grp_eva, self.femd_in.file_hdl.filename.split('/')[-1],
                                                     self.femd_in.list_emds[0])
            ...
            if len(self.dims) == 3:
                ...
                for i in range(len(self.radprof)):
                    if not self.radprof[i] is None:
                        ...
                        femd.put_emdgroup('radial_profile', radprof,
                                          ((self.radprof[n][:, 0], 'radial distance', self.dims[2][2]), self.dims[0]),
                                          parent=hdl, overwrite=True)
                        break
                    ...
                    if not self.res[i] is None:
                        ...
                        for ii in range(len(self.res)):
                            if not self.res[ii] is None:
                                res[:, ii] = self.res[ii][:]
                        femd.put_emdgroup('fit_results', res,
                                          ((np.array(range(res.shape[0])), 'parameters', '[]'), self.dims[0]),
                                          parent=hdl, overwrite=True)
                        break
                ...
                for i in range(len(self.center)):
                    if not self.center[i] is None:
                        ...
                        femd.put_emdgroup('centers', centers,
                                          ((np.array(range(2)), 'dimension', self.dims[2][2]), self.dims[0]),
                                          parent=hdl, overwrite=True)
                        break
                ...
                for i in range(len(self.dists)):
                    if not self.dists[i] is None:
                        ...
                        femd.put_emdgroup('distortions', dists,
                                          ((np.array(range(dists.shape[0])), 'parameters', '[]'), self.dims[0]),
                                          parent=hdl, overwrite=True)
                        break
                ...
                for i in range(len(self.back_params)):
                    if not self.back_params[i] is None:
                        ...
                        femd.put_emdgroup('back_results', back_params, (
                        (np.array(range(back_params.shape[0])), 'background parameters', '[]'), self.dims[0]),
                                          parent=hdl, overwrite=True)
                        break

            else:
                if not self.radprof[self.idx] is None:
                    femd.put_emdgroup('radial_profile', self.radprof[0][:, 1],
                                      ((self.radprof[0][:, 0], 'radial_distance', self.dims[0][2]),), parent=hdl,
                                      overwrite=True)
                if not self.res[self.idx] is None:
                    femd.put_emdgroup('fit_results', self.res[0],
                                      ((np.array(range(self.res[0].shape[0])), 'parameters', '[]'),), parent=hdl,
                                      overwrite=True)
                if not self.center[self.idx] is None:
                    femd.put_emdgroup('centers', self.center[0], ((np.array(range(2)), 'dimension', self.dims[0][2]),),
                                      parent=hdl, overwrite=True)
                if not self.dists[self.idx] is None:
                    femd.put_emdgroup('distortions', self.dists[0],
                                      ((np.array(range(self.dists[0].shape[0])), 'parameters', '[]'),), parent=hdl,
                                      overwrite=True)
                if not self.back_params[self.idx] is None:
                    femd.put_emdgroup('back_results', self.back_params[0],
                                      ((np.array(range(self.back_params[0].shape[0])), 'background parameters', '[]'),),
                                      parent=hdl, overwrite=True)
    ...
    def on_intensitySlider(self):
        ...
        min_data = np.min(self.data)
        max_data = np.max(self.data)
        self.settings['plt_imgminmax'] = (
        (min_val - min_data) / (max_data - min_data), (max_val - min_data) / (max_data - min_data))
    ...
    def update_localmax(self):
        ...
        if not self.data is None:
            ...
            if len(self.dims) == 2:
                ...
                # plot image
                self.plt_localmax_img = pg.ImageItem(self.data[self.idx, :, :].transpose((1, 0)).astype('float64'),
                                                     levels=(self.gui_localmax['min_slider'].value(),
                                                             self.gui_localmax['max_slider'].value()))
                self.plt_localmax_img.setZValue(-100)
                self.plt_localmax_img.setRect(
                    pg.QtCore.QRectF(self.dims[1][0][0], self.dims[0][0][0], self.dims[1][0][-1] - self.dims[1][0][0],
                                     self.dims[0][0][-1] - self.dims[0][0][0]))
                ...
                self.plt_localmax_img.setZValue(-100)
                self.plt_localmax_img.setRect(
                    pg.QtCore.QRectF(self.dims[2][0][0], self.dims[1][0][0], self.dims[2][0][-1] - self.dims[2][0][0],
                                     self.dims[1][0][-1] - self.dims[1][0][0]))
                self.plt_localmax.addItem(self.plt_localmax_img)

            # plot points
            if not self.points[self.idx] is None:
                # draw points
                self.plt_localmax.plot(self.points[self.idx][:, 1], self.points[self.idx][:, 0], pen=None, symbol='o',
                                       symbolPen=(255, 0, 0), symbolBrush=None)
    ...
    def on_localmax(self):
        ...
        try:
            ...
            raise TypeError('Bad input to local maximaxx')
        ...
        # find local max
        points = ncempy.algo.local_max.local_max(self.data[self.idx, :, :], self.settings['lmax_r'],
                                                 self.settings['lmax_thresh'])
        ...
        if len(self.settings['lmax_cinit']) == 2 and len(self.settings['lmax_range']) == 2:
            points = ncempy.algo.distortion.filter_ring(points, self.settings['lmax_cinit'],
                                                        self.settings['lmax_range'])
    ...
    def localmax_mouseClicked(self, evt):
        ...
        if self.gui_localmax['btn_lmax_cinit'].isChecked():
            ...
            self.settings['lmax_cinit'] = cinit
            self.log('Selected initial center guess: ({:g}, {:g}).'.format(self.settings['lmax_cinit'][0],
                                                                           self.settings['lmax_cinit'][1]))
            self.gui_localmax['txt_lmax_cinit'].setText(
                ', '.join(map('{:g}'.format, self.settings['lmax_cinit'][::-1])))
    ...
    def update_polar(self):
        ...
        if not self.center[self.idx] is None:
            ...
            self.gui_polar['center_lbl'].setText(
                'center: ({:.3f}, {:.3f})'.format(self.center[self.idx][1], self.center[self.idx][0]))

            if not self.points[self.idx] is None:
                points_plr = ncempy.algo.distortion.points_topolar(self.points[self.idx], self.center[self.idx])

                # horizontal mean line
                self.plt_polar.plot([-np.pi, np.pi], [np.mean(points_plr[:, 0]), np.mean(points_plr[:, 0])],
                                    pen=pg.mkPen('k', style=QtCore.Qt.DashLine))

                # uncorrected radial positions
                self.plt_polar.plot(points_plr[:, 1], points_plr[:, 0], pen=None, symbol='x', symbolPen=(255, 0, 0),
                                    symbolBrush=None)
                ...
                if not self.dists[self.idx] is None:
                    ...
                    for i in range(len(self.settings['ns'])):
                        self.plt_polar.plot(xpl_ell, self.dists[self.idx][0] * ncempy.algo.distortion.rad_dis(xpl_ell,
                                                                                                              self.dists[
                                                                                                                  self.idx][
                                                                                                                  i * 2 + 1],
                                                                                                              self.dists[
                    ...
                    for i in range(len(self.settings['ns'])):
                        sum_dists *= ncempy.algo.distortion.rad_dis(xpl_ell, self.dists[self.idx][i * 2 + 1],
                                                                    self.dists[self.idx][i * 2 + 2],
                                                                    self.settings['ns'][i])
                    ...
                    for i in range(len(self.settings['ns'])):
                        points_plr_corr[:, 0] /= ncempy.algo.distortion.rad_dis(points_plr_corr[:, 1],
                                                                                self.dists[self.idx][i * 2 + 1],
                                                                                self.dists[self.idx][i * 2 + 2],
                                                                                self.settings['ns'][i])

                    self.plt_polar.plot(points_plr_corr[:, 1], points_plr_corr[:, 0], pen=None, symbol='x',
                                        symbolPen=(0, 180, 0), symbolBrush=None)
    ...
    def on_copyCenter(self):
        ...
        if len(cinit) == 0:
            ...
            self.log(
                'Copied initial center guess: ({:g}, {:g}).'.format(self.center[self.idx][0], self.center[self.idx][1]))
    ...
    def on_fitDist(self):
        ...
        if len(ns) >= 1:
            ...
            for i in range(len(self.settings['ns'])):
                self.log('.. .. order: {:d}, alpha: {:g}, beta: {:g}'.format(self.settings['ns'][i],
                                                                             self.dists[self.idx][1 + i * 2],
                                                                             self.dists[self.idx][2 + i * 2]))
        ...
        if (not self.radprof is None) and (not self.radprof[self.idx] is None):
            ...
            if (not self.res[self.idx] is None) and (self.gui_radprof['fit_check'].isChecked()):
                ...
                for n in range(len(self.settings['fit_funcs'])):
                    self.plt_radprof.plot(R, ncempy.algo.math.lkp_funcs[self.settings['fit_funcs'][n]][0](R, self.res[
                                                                                                                 self.idx][
                                                                                                             i:i +
                                                                                                               ncempy.algo.math.lkp_funcs[
                                                                                                                   self.settings[
                ...
                i = 0
                for n in range(len(self.settings['fit_funcs'])):
                    self.plt_radprof.plot(R, ncempy.algo.math.lkp_funcs[self.settings['fit_funcs'][n]][0](R,
                                                                                                          self.settings[
                                                                                                              'fit_init'][
                                                                                                          i:i +
                                                                                                            ncempy.algo.math.lkp_funcs[
    ...
    def on_extractRadProf(self):
        ...
        # save settings
        if len(pars) == 0:
            self.settings['rad_rmax'] = np.abs(c_dims[1][0][0] - c_dims[1][0][1]) * np.min(
                self.data[self.idx, :, :].shape) / 2.0
            self.settings['rad_dr'] = np.abs(c_dims[1][0][0] - c_dims[1][0][1]) / 10.
            self.settings['rad_sigma'] = np.abs(c_dims[1][0][0] - c_dims[1][0][1])
            self.log(
                '.. calculating adaptive defaults: r_max: {:g}, dr: {:g}, sigma: {:g}'.format(self.settings['rad_rmax'],
                                                                                              self.settings['rad_dr'],
                                                                                              self.settings[
                                                                                                  'rad_sigma']))
        else:
        ...
        if self.gui_radprof['crct_check'].isChecked():
            rs, thes = ncempy.algo.radial_profile.calc_polarcoords(self.center[self.idx], c_dims, self.settings['ns'],
                                                                   self.dists[self.idx])
        ...
        R, I = ncempy.algo.radial_profile.calc_radialprofile(self.data[self.idx, :, :], rs, self.settings['rad_rmax'],
                                                             self.settings['rad_dr'], self.settings['rad_sigma'],
                                                             self.mask)
    ...
    def on_subtractBackground(self):
        ...
        res_back = ncempy.algo.radial_profile.fit_radialprofile(fit_R, fit_I, tuple(funcs_back),
                                                                self.settings['back_init'], maxfev=1000)

        # save output
        self.back_params[self.idx] = res_back

        self.log('.. background fitted to: offset: {:g}, amplitude: {:g}, exponent: {:g}'.format(
            self.back_params[self.idx][0], self.back_params[self.idx][1], self.back_params[self.idx][2]))
    ...
    def on_fitRadProf(self):
        ...
        if not self.radprof is None:
            ...
            if not len(self.settings['fit_rrange']) == 0:
                ...
                I = I[sel]
                R = R[sel]
                self.log('.. fitting in the range of {:g} to {:g}.'.format(self.settings['fit_rrange'][0],
                                                                           self.settings['fit_rrange'][1]))
            ...
            res = ncempy.algo.radial_profile.fit_radialprofile(R, I, self.settings['fit_funcs'],
                                                               self.settings['fit_init'], 10000)

            self.res[self.idx] = res
            self.log('.. peaks fitted to:')
            n = 0
            for i in range(len(self.settings['fit_funcs'])):
                self.log('.. .. {}: {}'.format(self.settings['fit_funcs'][i], ', '.join(map('{:g}'.format,
                                                                                            self.res[self.idx][n:n +
                                                                                                                 ncempy.algo.math.lkp_funcs[
                                                                                                                     self.settings[
                                                                                                                         'fit_funcs'][
                                                                                                                         i]][
    ...
    def on_saveCorrPattern(self):
        ...
        try:
            self.log('Saving corrected diffraction pattern to file "{}".'.format(fname[0]))

            # create/overwrite outfile
            if os.path.isfile(fname[0]):
                os.remove(fname[0])
            femd = ncempy.io.emd.fileEMD(fname[0])
            ...
            for i in range(self.data.shape[0]):
                ...
                if (not self.center[self.idx] is None) and (not self.dists[self.idx] is None):
                    ...
                    data_corr[self.idx, :, :] = ncempy.algo.radial_profile.correct_distortion(self.data[self.idx, :, :],
                                                                                              c_dims,
                    ...
                    self.log(
                        '.. skipping image {:d}/{:d} for missing results.'.format(self.idx + 1, self.data.shape[0]))

            if len(self.dims) == 3:
                femd.put_emdgroup(self.femd_in.file_hdl.filename.split('/')[-1], data_corr, self.dims, parent=grp)
            else:
                femd.put_emdgroup(self.femd_in.file_hdl.filename.split('/')[-1], data_corr[self.idx, :, :], self.dims,
                                  parent=grp)
